...
for s in all_structs_arranged:
    ...
    if len(all_used_structs) > 0:
        ...
        for struct in all_used_structs:
            ...
            if struct["type"] == "TagDependency":
                ...
                cpp_read_cache_file_data.write("                    eprintf(\"corrupt tag reference (class in reference does not match class in referenced tag)\\n\");\n")
                ...
                cpp_read_cache_file_data.write("            }\n")
                cpp_read_cache_file_data.write("            catch (std::exception &) {\n")
                cpp_read_cache_file_data.write("                eprintf(\"failed to read tag data for {}.{} in %s.%s\\n\", tag.get_path().data(), HEK::tag_class_to_extension(tag.get_tag_class_int()));\n".format(struct_name, name))
                cpp_read_cache_file_data.write("                throw;\n")
                cpp_read_cache_file_data.write("            }\n")
    ...
    cpp_read_hek_data.write("                eprintf(\"failed to read {} base struct: %zu bytes needed > %zu bytes available\\n\", sizeof(struct_big), data_size);\n".format(struct_name))
    ...
    if len(all_used_structs) > 0:
        ...
        for struct in all_used_structs:
            ...
            if struct["type"] == "TagDependency":
                ...
                cpp_read_hek_data.write("                eprintf(\"failed to read dependency {}.{}: %zu bytes needed > %zu bytes available\\n\", h_{}_expected_length, data_size);\n".format(struct_name, name, name))
                cpp_read_hek_data.write("                throw OutOfBoundsException();\n")
                cpp_read_hek_data.write("            }\n")
                ...
                cpp_read_hek_data.write("                    eprintf(\"failed to read dependency {}.{}: size is smaller than expected (%zu expected > %zu actual)\\n\", h_{}_expected_length, i);\n".format(struct_name, name, name))
                ...
                cpp_read_hek_data.write("                eprintf(\"failed to read dependency {}.{}: missing null terminator\\n\");\n".format(struct_name, name))
                cpp_read_hek_data.write("                throw InvalidTagDataException();\n")
                cpp_read_hek_data.write("            }\n")
                ...
                cpp_read_hek_data.write("                eprintf(\"failed to read reflexive {}.{}: %zu bytes needed > %zu bytes available\\n\", total_size, data_size);\n".format(struct_name, name))
                cpp_read_hek_data.write("                throw OutOfBoundsException();\n")
                cpp_read_hek_data.write("            }\n")
                ...
                cpp_read_hek_data.write("            eprintf(\"failed to read tag data block {}.{}: %zu bytes needed > %zu bytes available\\n\", h_{}_size, data_size);\n".format(struct_name, name, name))
                cpp_read_hek_data.write("            throw OutOfBoundsException();\n")
                cpp_read_hek_data.write("        }\n")
    ...
    cpp_read_hek_data.write("            eprintf(\"data is too small to have a header\\n\");")
    cpp_read_hek_data.write("            throw OutOfBoundsException();\n")
    cpp_read_hek_data.write("        }\n")
    ...
    cpp_read_hek_data.write("            eprintf(\"invalid tag file; tag data was left over\\n\");")
    cpp_read_hek_data.write("            throw InvalidTagDataException();\n")
    cpp_read_hek_data.write("        }\n")
    cpp_read_hek_data.write("        return r;\n")
    cpp_read_hek_data.write("    }\n")
