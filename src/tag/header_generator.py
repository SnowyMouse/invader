    ...
    if len(all_used_structs) > 0:
        ...
        for struct in all_used_structs:
            ...
            if struct["type"] == "TagDependency":
                ...
                cpp.write("            b.{}.count = ref_{}_size;\n".format(name, name))
                cpp.write("            auto total_size = sizeof({}::struct_big) * ref_{}_size;\n".format(struct["struct"], name))
                cpp.write("            converted_data.insert(converted_data.end(), total_size, std::byte());\n")
                cpp.write("            auto *ref_items = reinterpret_cast<{}::struct_big *>(converted_data.data() + converted_data.size() - total_size);\n".format(struct["struct"], name))
                ...
                cpp.write("                ref_items[i] = *reinterpret_cast<const {}::struct_big *>(struct_data);\n".format(struct["struct"]))
                cpp.write("                converted_data.insert(converted_data.end(), struct_data + sizeof(*ref_items), struct_data + converted_struct.size());\n")
                cpp.write("            }\n")
                cpp.write("        }\n")
