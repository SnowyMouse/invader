...
#include <invader/tag/parser/definition/detail_object_collection.hpp>
...
namespace Invader::Parser {
    void DetailObjectCollection::post_compile(BuildWorkload &workload, std::size_t tag_index, std::size_t struct_index, std::size_t offset) {
        ...
        auto &struct_data = *reinterpret_cast<C<LittleEndian> *>(this_struct.data.data() + offset);
        ...
        auto *types = reinterpret_cast<DetailObjectCollectionObjectType::C<LittleEndian> *>(workload.structs[*this_struct.resolve_pointer(&struct_data.types.pointer)].data.data());
        ...
        auto &sprite_plate_data = *reinterpret_cast<Bitmap::C<LittleEndian> *>(sprite_plate_struct.data.data());
        ...
        auto *sequences = sequences_struct_index.has_value() ? reinterpret_cast<BitmapGroupSequence::C<LittleEndian> *>(workload.structs[*sequences_struct_index].data.data()) : nullptr;
        ...
        for(std::size_t t = 0; t < type_count; t++) {
            ...
            if(type.sequence_index >= sequence_count) {
                if(!workload.disable_error_checking) {
                    REPORT_ERROR_PRINTF(workload, ERROR_TYPE_FATAL_ERROR, tag_index, "Sequence index #%zu of bitmap %s.%s for type #%zu is out of bounds (%zu >= %zu)", sequence_index, File::halo_path_to_preferred_path(sprite_plate_tag.path).c_str(), tag_fourcc_to_extension(sprite_plate_tag.tag_fourcc), sequence_index, sequence_index, sequence_count);
                ...
                if(!workload.disable_error_checking) {
                    if(sprite_count > MAX_SPRITE_COUNT) {
                        REPORT_ERROR_PRINTF(workload, ERROR_TYPE_FATAL_ERROR, tag_index, "Sequence index #%zu of bitmap %s.%s for type #%zu exceeds the maximum number of sprites allowed for detail object collections (%zu > %zu)", sequence_index, File::halo_path_to_preferred_path(sprite_plate_tag.path).c_str(), tag_fourcc_to_extension(sprite_plate_tag.tag_fourcc), sequence_index, static_cast<std::size_t>(sprite_count), MAX_SPRITE_COUNT);
                        throw InvalidTagDataException();
                    }
                    if(sequence_offset > MAX_SPRITE_COUNT) {
                        REPORT_ERROR_PRINTF(workload, ERROR_TYPE_FATAL_ERROR, tag_index, "Sequence index #%zu of bitmap %s.%s for type #%zu has a sprite offset which exceeds the maximum number of sprites allowed for detail object collections (%zu > %zu)", sequence_index, File::halo_path_to_preferred_path(sprite_plate_tag.path).c_str(), tag_fourcc_to_extension(sprite_plate_tag.tag_fourcc), sequence_offset, static_cast<std::size_t>(sprite_count), MAX_SPRITE_COUNT);
                        throw InvalidTagDataException();
                    }
                }
            }
