...
#include <invader/tag/parser/definition/unit_hud_interface.hpp>
...
#define CHECK_INTERFACE_BITMAP_SEQ(bitmap_tag, sequence_index, name) { \
    if((!workload.disable_recursion && !workload.disable_error_checking) && (!bitmap_tag.tag_id.is_null())) { \
        ...
        if(sequence_index >= sequence_count) { \
            ...
            if(bitmap_type == BitmapType::BITMAP_TYPE_SPRITES && sequence.sprites.count == 0) { \
            ...
            else if(bitmap_type != BitmapType::BITMAP_TYPE_SPRITES && sequence.bitmap_count == 0) { \
...
if((flags & HUDInterfaceScalingFlagsFlag::HUD_INTERFACE_SCALING_FLAGS_FLAG_USE_HIGH_RES_SCALE) && workload.get_build_parameters()->details.build_cache_file_engine == CacheFileEngine::CACHE_FILE_XBOX) { \
...
namespace Invader::Parser {
    void WeaponHUDInterface::pre_compile(BuildWorkload &, std::size_t, std::size_t, std::size_t) {
        union {
            std::uint32_t inty;
            WeaponHUDInterfaceCrosshairTypeFlags flaggy;
        } crosshair_types = {};
        ...
        if(!(this->crosshair_types & WeaponHUDInterfaceCrosshairTypeFlagsFlag::WEAPON_HUD_INTERFACE_CROSSHAIR_TYPE_FLAGS_FLAG_ZOOM_LEVEL)) {
            ...
            for(auto &c : this->crosshairs) {
                auto set_hud_for_zoom_flags = [&c, &crosshair_types]() {
                    for(auto &o : c.crosshair_overlays) {
                        if(
                            (o.flags & WeaponHUDInterfaceCrosshairOverlayFlagsFlag::WEAPON_HUD_INTERFACE_CROSSHAIR_OVERLAY_FLAGS_FLAG_DONT_SHOW_WHEN_ZOOMED) ||
                            (o.flags & WeaponHUDInterfaceCrosshairOverlayFlagsFlag::WEAPON_HUD_INTERFACE_CROSSHAIR_OVERLAY_FLAGS_FLAG_SHOW_ONLY_WHEN_ZOOMED) ||
                            (o.flags & WeaponHUDInterfaceCrosshairOverlayFlagsFlag::WEAPON_HUD_INTERFACE_CROSSHAIR_OVERLAY_FLAGS_FLAG_ONE_ZOOM_LEVEL)
                        ) {
                            crosshair_types |= WeaponHUDInterfaceCrosshairTypeFlagsFlag::WEAPON_HUD_INTERFACE_CROSSHAIR_TYPE_FLAGS_FLAG_ZOOM_LEVEL;
                            return true;
                        }
                    }
                    return false;
    ...
    void get_sequence_data(const BuildWorkload &workload, const TagID &tag_id, std::size_t &sequence_count, const BitmapGroupSequence::C<LittleEndian> *&sequences, char *bitmap_tag_path, std::size_t bitmap_tag_path_size, BitmapType &bitmap_type) {
        if(tag_id.is_null()) {
            sequence_count = 0;
            sequences = nullptr;
            ...
            std::snprintf(bitmap_tag_path, bitmap_tag_path_size, "%s.%s", bitmap_tag.path.c_str(), tag_fourcc_to_extension(bitmap_tag.tag_fourcc));
            ...
            const auto &bitmap = *reinterpret_cast<const Bitmap::C<LittleEndian> *>(bitmap_struct.data.data());
            bitmap_type = bitmap.type;
            sequence_count = bitmap.bitmap_group_sequence.count;
            sequences = sequence_count > 0 ? reinterpret_cast<const Parser::BitmapGroupSequence::C<LittleEndian> *>(workload.structs[*bitmap_struct.resolve_pointer(&bitmap.bitmap_group_sequence.pointer)].data.data()) : nullptr;
        }
    }
    ...
    void WeaponHUDInterfaceCrosshair::post_compile(BuildWorkload &workload, std::size_t tag_index, std::size_t, std::size_t struct_offset) {
        ...
        if(!workload.disable_recursion || !workload.disable_error_checking) {
            for(std::size_t i = 0; i < overlay_count; i++) {
                ...
                bool not_a_sprite = overlay.flags & WeaponHUDInterfaceCrosshairOverlayFlagsFlag::WEAPON_HUD_INTERFACE_CROSSHAIR_OVERLAY_FLAGS_FLAG_NOT_A_SPRITE;
                if(not_a_sprite && bitmap_type == BitmapType::BITMAP_TYPE_SPRITES) {
                    REPORT_ERROR_PRINTF(workload, ERROR_TYPE_FATAL_ERROR, tag_index, "Overlay #%zu of crosshair #%zu is marked as not a sprite, but %s is a sprite sheet", i, struct_offset / sizeof(WeaponHUDInterfaceCrosshair::C<LittleEndian>), bitmap_tag_path);
                    throw InvalidTagDataException();
                }
                else if(!not_a_sprite && bitmap_type != BitmapType::BITMAP_TYPE_SPRITES) {
                    REPORT_ERROR_PRINTF(workload, ERROR_TYPE_FATAL_ERROR, tag_index, "Overlay #%zu of crosshair #%zu is not marked as not a sprite, but %s is not a sprite sheet", i, struct_offset / sizeof(WeaponHUDInterfaceCrosshair::C<LittleEndian>), bitmap_tag_path);
                    throw InvalidTagDataException();
                }

                if(overlay.sequence_index != NULL_INDEX) {
                    if(overlay.sequence_index >= sequence_count) {
                        REPORT_ERROR_PRINTF(workload, ERROR_TYPE_FATAL_ERROR, tag_index, "Sequence #%zu in %s referenced in overlay #%zu of crosshair #%zu is out of bounds (>= %zu)", static_cast<std::size_t>(overlay.sequence_index), bitmap_tag_path, i, struct_offset / sizeof(WeaponHUDInterfaceCrosshair::C<LittleEndian>), sequence_count);
                    throw InvalidTagDataException();
                    }
                    else {
                        ...
                        if(not_a_sprite) {
                            if(sequence.bitmap_count == 0) {
                                REPORT_ERROR_PRINTF(workload, ERROR_TYPE_FATAL_ERROR, tag_index, "Sequence #%zu in %s referenced in overlay #%zu of crosshair #%zu has 0 bitmaps", static_cast<std::size_t>(overlay.sequence_index), bitmap_tag_path, i, struct_offset / sizeof(WeaponHUDInterfaceCrosshair::C<LittleEndian>));
                                throw InvalidTagDataException();
                            }
                        }
                        else {
                            if(sequence.sprites.count == 0) {
                                REPORT_ERROR_PRINTF(workload, ERROR_TYPE_FATAL_ERROR, tag_index, "Sequence #%zu in %s referenced in overlay #%zu of crosshair #%zu has 0 sprites", static_cast<std::size_t>(overlay.sequence_index), bitmap_tag_path, i, struct_offset / sizeof(WeaponHUDInterfaceCrosshair::C<LittleEndian>));
                                throw InvalidTagDataException();
                            }
                        }
                    }
