...
namespace Invader::Parser {
    void DamageEffect::pre_compile(BuildWorkload &workload, std::size_t tag_index, std::size_t, std::size_t) {
        ...
        if(*workload.cache_file_type == CacheFileType::SCENARIO_TYPE_SINGLEPLAYER && workload.tags[tag_index].path == "weapons\\pistol\\bullet") {
        ...
        if(workload.building_stock_map && (workload.tags[tag_index].path == "vehicles\\ghost\\ghost bolt" || workload.tags[tag_index].path == "vehicles\\banshee\\banshee bolt")) {
            bool custom_edition = workload.get_build_parameters()->details.build_cache_file_engine == CacheFileEngine::CACHE_FILE_CUSTOM_EDITION;
