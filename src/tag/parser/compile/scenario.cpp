    ...
    void Scenario::pre_compile(BuildWorkload &workload, std::size_t tag_index, std::size_t, std::size_t) {
        ...
        if(this->scripts.size() > 0) {
            if(this->source_files.size() == 0) {
                ...
                workload.report_error(BuildWorkload::ErrorType::ERROR_TYPE_WARNING, "To fix this, recompile the scripts", tag_index);
            }
            else {
            ...
            workload.report_error(BuildWorkload::ErrorType::ERROR_TYPE_WARNING, "TODO: Implement reference array rebuilding", tag_index);
            std::terminate();
        }
