...
def make_parse_hek_tag_data(definition, all_types, append_line):
    for s in definition["definitions"]:
        if s["type"] != "struct":
            continue
        struct_name = s["name"]
        append_line("{} {}::parse_hek_tag_data([[maybe_unused]] const std::byte *data, [[maybe_unused]] std::size_t data_size, [[maybe_unused]] std::size_t &cursor, [[maybe_unused]] const C<BigEndian> &h) {{".format(struct_name, struct_name))
        append_line("{} r = {{}};".format(struct_name), 1)
        
        # If it inherits anything, we should read that first
        if "inherits" in s:
            append_line("dynamic_cast<{} &>(r) = {}::parse_hek_tag_data(data, data_size, cursor, h);".format(s["inherits"], s["inherits"]), 1)
        ...
        for field in s["fields"]:
            ...
            if field["type"] == "TagDependency":
                ...
                append_line("}", 2)
                append_line("const char *h_{}_char = reinterpret_cast<const char *>(data + cursor);".format(name), 2)
                ...
                append_line("}", 3)
                append_line("}", 2)
                append_line("if(static_cast<char>(h_{}_char[h_{}_expected_length]) != 0) {{".format(name, name), 2)
                ...
                append_line("}", 2)
                if not unread:
                    append_line("r.{}.path = Invader::File::remove_duplicate_slashes(std::string(h_{}_char));".format(name, name), 2)
                append_line("cursor += h_{}_expected_length + 1;".format(name), 2)
                append_line("}", 1)
                if field["classes"][0] != "*":
                ...
                append_line("const auto *array = reinterpret_cast<const {}::C<BigEndian> *>(data + cursor);".format(field["struct"]), 2)
                append_line("std::size_t total_size = sizeof(*array) * h_{}_count;".format(name), 2)
                append_line("if(cursor + total_size > data_size) {", 2)
                append_line("eprintf_error(\"Failed to read reflexive {}::{}: %zu bytes needed > %zu bytes available\", total_size, data_size - cursor);".format(struct_name, name), 3)
                append_line("throw OutOfBoundsException();", 3)
                append_line("}", 2)
                append_line("cursor += total_size;", 2)
                if not unread:
                ...
                call = "{}::parse_hek_tag_data(data, data_size, cursor, array[ref])".format(field["struct"])
                if not unread:
                    append_line("r.{}.emplace_back({});".format(name, call), 3)
                else:
                    append_line("{};".format(call), 3)
                append_line("cursor += ref_data_read;", 3)
                append_line("}", 2)
                append_line("}", 1)
            elif field["type"] == "TagDataOffset":
                append_line("std::size_t h_{}_size = h.{}.size;".format(name, name), 1)
                append_line("if(h_{}_size + cursor > data_size) {{".format(name), 1)
                append_line("eprintf_error(\"Failed to read tag data block {}::{}: %zu bytes needed > %zu bytes available\", h_{}_size, data_size - cursor);".format(struct_name, name, name), 2)
                append_line("throw OutOfBoundsException();", 2)
                append_line("}", 1)
                if not unread:
                    append_line("r.{} = std::vector<std::byte>(data + cursor, data + h_{}_size + cursor);".format(name, name), 1)
                append_line("cursor += h_{}_size;".format(name), 1)
