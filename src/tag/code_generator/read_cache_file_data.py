    ...
    if len(all_used_structs) > 0:
        ...
        for struct in all_used_structs:
            ...
            if struct["type"] == "TagDependency":
                ...
                cpp_read_cache_file_data.write("                eprintf_error(\"Invalid reference for {}::{} in %s.%s\", File::halo_path_to_preferred_path(tag.get_path()).c_str(), HEK::tag_class_to_extension(tag.get_tag_fourcc()));\n".format(struct_name, name))
                ...
                cpp_read_cache_file_data.write("                }\n")
                cpp_read_cache_file_data.write("                catch (std::exception &) {\n")
                cpp_read_cache_file_data.write("                    eprintf_error(\"Failed to parse {}::{} #%zu in %s.%s\", i, File::halo_path_to_preferred_path(tag.get_path()).c_str(), HEK::tag_class_to_extension(tag.get_tag_fourcc()));\n".format(struct_name, name))
                cpp_read_cache_file_data.write("                    throw;\n")
                cpp_read_cache_file_data.write("                }\n")
                cpp_read_cache_file_data.write("            }\n")
                cpp_read_cache_file_data.write("        }\n")
                ...
                cpp_read_cache_file_data.write("            }\n")
                cpp_read_cache_file_data.write("            catch (std::exception &) {\n")
                cpp_read_cache_file_data.write("                eprintf_error(\"Failed to read tag data for {}::{} in %s.%s\", File::halo_path_to_preferred_path(tag.get_path()).c_str(), HEK::tag_class_to_extension(tag.get_tag_fourcc()));\n".format(struct_name, name))
                cpp_read_cache_file_data.write("                throw;\n")
                cpp_read_cache_file_data.write("            }\n")
