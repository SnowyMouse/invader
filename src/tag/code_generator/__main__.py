...
if len(sys.argv) < bitfield_cpp+4:
    print("Usage: {} <definition.hpp> <parser.hpp> <parser-save-hek-data.cpp> <parser-read-hek-data.cpp> <parser-read-cache-file-data.cpp> <parser-cache-format.cpp> <parser-cache-deformat.cpp> <parser-refactor-reference.cpp> <parser-struct-value.cpp> <parser-check-broken-enums.cpp> <parser-check-invalid-references.cpp> <parser-check-invalid-ranges.cpp> <parser-normalize.cpp> <parser-read-hek-file.cpp> <parser-check-uppercase-references.cpp> <bitfield.cpp> <enum.cpp> <extract-hidden> <json> [json [...]]".format(sys.argv[0]), file=sys.stderr)
    sys.exit(1)
        ...
        if s["type"] == "enum":
            ...
            for f in s["fields"]:
                ...
                if f["type"] == "TagDependency":
                    ...
                    expand_superclass(f["classes"], "bitmap", ["invader_bitmap"])
                    expand_superclass(f["classes"], "model", ["gbxmodel"])
                    expand_superclass(f["classes"], "sound", ["invader_sound"])
