...
namespace Invader::File {
    ...
    std::optional<std::string> attempt_to_resolve_tag_path(const std::string &tag_path, const std::vector<std::string> &tags, const std::optional<std::string> expected_extension) {
        if(expected_extension.has_value()) {
            const auto &EXTENSION = expected_extension.value();

            // Do we have it?
            auto tag_path_with_extension = tag_path + EXTENSION;
            if(tag_path_to_file_path(tag_path_with_extension, tags, true).has_value()) {
                return tag_path;
            }

            std::size_t EXTENSION_LENGTH = EXTENSION.size();
            ...
            if(TAG_PATH_LENGTH > EXTENSION_LENGTH && std::strcmp(tag_path.data() + TAG_PATH_LENGTH - EXTENSION_LENGTH, EXTENSION.data()) == 0) {
            ...
            if(tag_path_to_file_path(tag_path, tags, true).has_value()) {
                return tag_path;
            }
            else {
                return file_path_to_tag_path(tag_path, tags, true);
            }
        }
    }
