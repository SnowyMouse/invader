...
namespace Invader {
    ...
    std::byte *Tag::data(HEK::Pointer64 pointer, std::size_t minimum) {
        ...
        if(this->tag_fourcc == TagClassInt::TAG_CLASS_SOUND && this->indexed) {
        ...
        if(this->indexed || (this->get_map().get_engine() != HEK::CacheFileEngine::CACHE_FILE_NATIVE && this->tag_fourcc == TagClassInt::TAG_CLASS_SCENARIO_STRUCTURE_BSP && pointer >= this->base_struct_pointer)) {
