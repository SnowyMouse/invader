...
namespace Invader {
    ...
    public:
        ...
        /**
         * Return the number of bytes in tag data
         * @return the number of bytes in tag data
         */
        std::size_t data_size() const noexcept;

        /**
         * Get a pointer to the asset data, optionally guaranteeing that a set amount of bytes is valid.
         * @param  minimum minimum number of bytes to guarantee to be valid
         * @return         pointer to the data; nullptr if no data and minimum is not set
         * @throws         throw if invalid and minimum is set or if there are fewer bytes valid than minimum
         */
        std::byte *asset_data(std::size_t minimum = 0);
         ...
         * @throws         throw if invalid and minimum is set or if there are fewer bytes valid than minimum
         */
        const std::byte *asset_data(std::size_t minimum = 0) const;

        /**
         * Return the number of bytes in tag data
         * @return the number of bytes in tag data
         */
        std::size_t asset_data_size() const noexcept;
         ...
         */
         ...
         StructType<HEK::LittleEndian> &get_struct_at_offset(std::size_t offset, std::size_t minimum_size = sizeof(StructType<HEK::LittleEndian>)) {
             if(offset > this->p_tag_data.size() || minimum_size + offset > this->p_tag_data.size()) {
                 throw OutOfBoundsException();
             }
             return *reinterpret_cast<StructType<HEK::LittleEndian> *>(this->data(sizeof(StructType<HEK::LittleEndian>) + offset) + offset);
         }
        ...
        StructType<HEK::LittleEndian> *resolve_reflexive(const HEK::TagReflexive<HEK::LittleEndian, StructType> &reflexive) {
            ...
            return &get_struct_at_offset<StructType>(reflexive.pointer.read() - this->base_address, sizeof(StructType<HEK::LittleEndian>) * reflexive.count);
        }
    ...
    private:
        ...
        /** Data of tag */
        std::vector<std::byte> p_tag_data;

        /** Asset data of tag */
        std::vector<std::byte> p_asset_data;
